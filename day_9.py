...
def day_9(data):
    ...
    def checkDepth(n):
        ...
        trues = 0
        for n in [getDepth(index, i+1), getDepth(index,i-1), getDepth( index-1 ,i), getDepth(index+1,i)]:
            if(data[index][i] < n):
                trues += 1
    ...
    for index in range(len(data)):
        for i in range(len(data[index])):
            risk += data[index][i] + 1 if checkDepth(data[index][i]) else 0
